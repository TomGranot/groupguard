...
const OUTPUT_START_MARKER = '---GROUPGUARD_OUTPUT_START---';
const OUTPUT_END_MARKER = '---GROUPGUARD_OUTPUT_END---';
...
function formatTranscriptMarkdown(messages: ParsedMessage[], title?: string | null): string {
  ...
  for (const msg of messages) {
    const sender = msg.role === 'user' ? 'User' : 'GroupGuard';
...
async function main(): Promise<void> {
  ...
  if (input.isScheduledTask) {
    prompt = `[SCHEDULED TASK - You are running automatically, not in response to a user message. Use mcp__groupguard__send_message if needed to communicate with the user.]\n\n${input.prompt}`;
  }
