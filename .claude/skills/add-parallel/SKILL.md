...
Adds Parallel AI MCP integration to GroupGuard for advanced web research capabilities.
...
- **Non-blocking Design** - Uses GroupGuard scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. GroupGuard already set up and running
...
Add Parallel AI MCP servers after the groupguard server:
```typescript
const mcpServers: Record<string, any> = {
  groupguard: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__groupguard__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__groupguard__send_message
   4. Use mcp__groupguard__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo groupguard-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/groupguard.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.groupguard`
