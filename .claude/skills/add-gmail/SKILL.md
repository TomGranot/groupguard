---
name: add-gmail
description: Add Gmail integration to GroupGuard. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to GroupGuard. It can be configured in two modes:
...
> How do you want to use Gmail with GroupGuard?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "GroupGuard"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "GroupGuard Gmail")
>    - Click **Create**
...
```bash
launchctl kickstart -k gui/$(id -u)/com.groupguard
```

Check that it started:

```bash
sleep 2 && launchctl list | grep groupguard
```
...
> - Create a Gmail label (e.g., "GroupGuard")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'GroupGuard',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/groupguard.log | grep -i email
```
...
```bash
tail -f logs/groupguard.log | grep -E "(email|Email)"
```
