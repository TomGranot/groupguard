...
# GroupGuard Container Debugging
...
|-----|----------|---------|
| **Main app logs** | `logs/groupguard.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/groupguard.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash groupguard-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash groupguard-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash groupguard-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash groupguard-agent:latest
```
...
# Also clear the session ID from GroupGuard's tracking
echo '{}' > data/sessions.json
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/groupguard.log | tail -5
...
```bash
echo "=== Checking GroupGuard Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo groupguard-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/groupguard.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
