...
function parseGuard(value: unknown, label: string): ModerationGuardConfig {
  ...
  if (input.params !== undefined) record(input.params, `${label}.params`);
  return {
    id: input.id as ModerationGuardConfig['id'],
    enabled: input.enabled,
    ...(input.params ? { params: input.params as Record<string, unknown> } : {}),
  };
}
...
function parseModeration(value: unknown, label: string): GroupModerationConfig {
  ...
  return {
    ...
    notifyOnDelete: bool(input.notifyOnDelete, false, `${label}.notifyOnDelete`),
...
function parseDirectory(value: unknown): GroupDirectoryConfig {
  ...
  return {
    ...
    ollamaBaseUrl: input.ollamaBaseUrl === undefined
...
export function parseGroupGuardConfig(value: unknown): GroupGuardConfig {
  ...
  for (const [groupId, candidate] of Object.entries(groupsInput)) {
    ...
    groups[groupId] = {
      ...
      forwardUnmatchedToAgent: bool(
        group.forwardUnmatchedToAgent,
        false,
        `groups.${groupId}.forwardUnmatchedToAgent`,
      ),
